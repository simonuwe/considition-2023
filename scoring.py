...
def setNeighbors(neighbors):
    global scoreNeighbors
    scoreNeighbors = copy.deepcopy(neighbors)
    # print('setNeighbors')
...
def calculateScore(mapName, solution, mapEntity, generalData):
    ...
    for key in scoredSolution[LK.locations]:
        ...
        loc[SK.total] = loc[LK.co2Savings]/1000 * generalData[GK.co2PricePerKiloInSek] + loc[SK.earnings]/1000
    ...
    scoredSolution[SK.gameScore][SK.totalFootfall] = round(
        scoredSolution[SK.gameScore][SK.totalFootfall], 6
    )
...
def distributeSales(with_, without, generalData):
    # pprint(scoreNeighbors)
    for key_without in without:
        distributeSalesTo = {}
        loc_without = without[key_without]

        for key_with_ in with_:
            # print('distance', key_without, key_with_)
            if  key_with_ in scoreNeighbors[key_without]["neighbors"]:
                distance = scoreNeighbors[key_without]["neighbors"][key_with_]["distance"]
            else:
                distance = generalData[GK.willingnessToTravelInMeters]
            """
            distance1 = distanceBetweenPoint(
                loc_without[CK.latitude],
                loc_without[CK.longitude],
                with_[key_with_][CK.latitude],
                with_[key_with_][CK.longitude],
            )
            if distance1 != distance and distance<generalData[GK.willingnessToTravelInMeters]:
                print ('neighbor mismatch', key_without, key_with_, distance, distance1)
            """
        ...
        if distributeSalesTo:
            ...
            # print('DISTRIBUTE', key_without, loc_without[LK.salesVolume], total)
    return with_
    ...
    for key in locations:
        ...
        for keySurrounding in locations:
            if key != keySurrounding:
                ...
                if distance < generalData[GK.willingnessToTravelInMeters]:
                    # print("initial too near", key, keySurrounding)
                    count += 1
...
def divideFootfall(locations, generalData):
    for key in locations:
        ...
        for keySurrounding in locations:
            if key != keySurrounding:
                if  keySurrounding in scoreNeighbors[key]["neighbors"]:
                    distance = scoreNeighbors[key]["neighbors"][keySurrounding]["distance"]
                else:
                    distance = generalData[GK.willingnessToTravelInMeters]
                """
                distance1 = distanceBetweenPoint(
                ...
                """
                if distance < generalData[GK.willingnessToTravelInMeters]:
                    # print("divide too near", key, keySurrounding)
                    count += 1
        # print('DIVIDE', key, locations[key][LK.footfall], count)
